...
class pBubble
{
    ...
    function drawBubbleChart($DataSeries, $WeightSeries, $Format = "")
    {
        ...
        foreach ($DataSeries as $Key => $SerieName) {
            ...
            foreach ($Data["Series"][$SerieName]["Data"] as $iKey => $Point) {
                ...
                $PosArray    = $this->pChartObject->scaleComputeY($Point, array(
                    "AxisID" => $AxisID
                ));
                $WeightArray = $this->pChartObject->scaleComputeY($Weight, array(
                    "AxisID" => $AxisID
                ));
    ...
    function writeBubbleLabel($SerieName, $SerieWeightName, $Points, $Format = "")
    {
        ...
        foreach ($Points as $Key => $Point) {
            ...
            $PosArray = $this->pChartObject->scaleComputeY($Value, array(
                "AxisID" => $AxisID
            ));
