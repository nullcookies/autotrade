...
class pPie
{
    ...
    function draw3DPie($X, $Y, $Format = "")
    {
        ...
        foreach ($Slices as $SliceID => $Plots) {
            ...
            if ($Visible[$SliceID]["Start"] && isset($Plots[2])) /* Empty error handling */ {
                $this->pChartObject->drawLine($Plots[2], $Plots[3], $Plots[2], $Plots[3] - $SliceHeight, array(
                    "R" => $Settings["R"],
                    "G" => $Settings["G"],
                    "B" => $Settings["B"]
                ));
        ...
        foreach ($Slices as $SliceID => $Plots) {
            ...
            if ($Visible[$SliceID]["End"]) {
                $this->pChartObject->drawLine($Plots[count($Plots) - 2], $Plots[count($Plots) - 1], $Plots[count($Plots) - 2], $Plots[count($Plots) - 1] - $SliceHeight, array(
                    "R" => $Settings["R"],
                    "G" => $Settings["G"],
                    "B" => $Settings["B"]
                ));
    ...
    function drawPieLegend($X, $Y, $Format = "")
    {
        ...
        foreach ($Data["Series"][$Data["Abscissa"]]["Data"] as $Key => $Value) {
            ...
            $this->pChartObject->drawFilledRectangle($X + 1, $Y + 1, $X + $BoxSize + 1, $Y + $BoxSize + 1, array(
                "R" => 0,
                "G" => 0,
                "B" => 0,
                "Alpha" => 20
            ));
            $this->pChartObject->drawFilledRectangle($X, $Y, $X + $BoxSize, $Y + $BoxSize, array(
                "R" => $R,
                "G" => $G,
                "B" => $B,
                "Surrounding" => 20
            ...
            if ($Mode == LEGEND_VERTICAL) {
                $this->pChartObject->drawText($X + $BoxSize + 4, $Y + $BoxSize / 2, $Value, array(
                    "R" => $FontR,
                    "G" => $FontG,
                    "B" => $FontB,
                ...
                $BoxArray = $this->pChartObject->drawText($X + $BoxSize + 4, $Y + $BoxSize / 2, $Value, array(
                    "R" => $FontR,
                    "G" => $FontG,
                    "B" => $FontB,
                    "Align" => TEXT_ALIGN_MIDDLELEFT,
    ...
    function writeShiftedLabels()
    {
        ...
        foreach ($this->LabelPos as $Key => $Settings) {
            ...
            $this->pChartObject->drawArrow($X2, $Y2, $X1, $Y1, array(
                "Size" => 8
            ));
            if ($Angle <= 180) {
                $this->pChartObject->drawLine($X2, $Y2, $X3, $Y2);
                $this->pChartObject->drawText($X3 + 2, $Y2, $Label, array(
                    "Align" => TEXT_ALIGN_MIDDLELEFT
                ));
            } else {
                $this->pChartObject->drawLine($X2, $Y2, $X3, $Y2);
    ...
    function draw2DRing($X, $Y, $Format = "")
    {
        ...
        if ($WriteValues && !$Shadow) {
            ...
            foreach ($Values as $Key => $Value) {
                ...
                $this->pChartObject->drawText($Xc, $Yc, $Display, array(
                    "Align" => $Align,
                    "R" => $ValueR,
                    "G" => $ValueG,
                    "B" => $ValueB
